...
private[spark] class TorrentBroadcastFactory extends BroadcastFactory {
  ...
  override def newBroadcast[T: ClassTag](value_ : T, isLocal: Boolean, id: Long): Broadcast[T] = {
    new TorrentBroadcast[T, T](value_, id, false)
  }

  override def newBroadcast[T: ClassTag, U: ClassTag](
      value: T,
      isLocal: Boolean,
      id: Long,
      isExecutorSide: Boolean,
      transFunc: TransFunc[U, T]): Broadcast[T] = {
    new TorrentBroadcast[T, U](value, id, isExecutorSide, Option(transFunc))
  }
